...
Regardless of whether you are personally on team light or team dark, it's becoming increasingly common for apps to support these two color schemes. Here is an example on how supporting both modes looks in an Expo project:
...
In development builds, you'll need to install the native package [`expo-system-ui`](/versions/latest/sdk/system-ui/#installation) otherwise the `userInterfaceStyle` property will be ignored. You can also run `npx expo config --type introspect` to check if the project is misconfigured. If it is, you'll see a warning as shown below:
...
- If working with an iOS emulator locally, you can use the <kbd>Cmd ⌘</kbd> + <kbd>Shift</kbd> + <kbd>a</kbd> shortcut to toggle between light and dark mode.

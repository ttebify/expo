...
If you need to look at release builds of your project, it is convenient not to overwrite the development build of your app every time you do so. You can accomplish this by using [**app.config.js**](/workflow/configuration) to set the bundle identifier or package name based on an environment variable. When changing the ID of your project, be aware that some modules will expect you to perform installation steps for each bundle identifier or package name you use. For more information, see [how to use this pattern on EAS Build with build variants](/build-reference/variants).

...
class MockErrorRecoveryDelegate: EXUpdatesErrorRecoveryDelegate {
  public enum Method {
    case relaunch
...
class EXUpdatesErrorRecoverySpec : ExpoSpec {
  override func spec() {
    func setUp() -> (DispatchQueue, EXUpdatesErrorRecovery) {
      let testQueue = DispatchQueue(label: "expo.errorRecoveryTestQueue")
      return (testQueue, EXUpdatesErrorRecovery(errorRecoveryQueue: testQueue, diskWriteQueue: testQueue, remoteLoadTimeout: 500))
    }
    ...
    describe("handleError") {
      ...
      it("NewBrokenUpdateLoaded_UpdateAlreadyLaunchedSuccessfully") {
        ...
        let mockUpdate = EXUpdatesUpdate(
      ...
      it("CheckAutomaticallyNever") {
        let (testQueue, errorRecovery) = setUp()
        let config = EXUpdatesConfig.config(fromDictionary: [
          EXUpdatesConfig.EXUpdatesConfigCheckOnLaunchKey: EXUpdatesConfig.EXUpdatesConfigCheckOnLaunchValueNever
        ])
        let mockDelegate = MockErrorRecoveryDelegate(
          config: config,
          relaunchCompletionParams: (nil, true)
    ...
    describe("error log") {
      it("consume") {
        ...
        let errorLog = EXUpdatesErrorRecovery.consumeErrorLog()
